...
// Simple initialization to avoid conflicts
let db;
try {
  // If the user's config doesn't have a specific databaseId, it will use the default
  ...
  console.warn("Firestore getFirestore failed, falling back to initializeFirestore with long polling", e);
  const dbId = firebaseConfig.firestoreDatabaseId && firebaseConfig.firestoreDatabaseId !== "(default)" 
    ? firebaseConfig.firestoreDatabaseId 
    : undefined;
...
export async function testConnection() {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      // Other errors (like permission denied) mean we reached the server
      return true;
    }
  }
  return false;
