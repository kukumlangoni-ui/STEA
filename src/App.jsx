...
import { SponsoredAdsSection } from "./components/SponsoredAdsSection.jsx";
...
const W = ({ children }) => {
  const isMobile = useMobile();
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", padding: isMobile ? "0 12px" : "0 14px" }}>
      {children}
    </div>
  );
};
...
function AuthModal({ onClose, onUser }) {
  ...
  const saveUser = async (user, displayName, provider) => {
    ...
    try {
      ...
      let role = user.email?.toLowerCase() === ADMIN_EMAIL.toLowerCase() ? "admin"
  ...
  useEffect(() => {
    ...
    const unsubAds = onSnapshot(q, (snap) => {
      ...
      if (activePopup && !localStorage.getItem(`popup_shown_${activePopup.id}`)) {
        ...
        t = setTimeout(() => setShowPopup(true), 5000); // Show after 5 seconds
      }
    });
