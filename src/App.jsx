...
const NAV = [
  ...
  { id: "websites", label: "Website Solutions" },
...
function AuthModal({ onClose, onUser }) {
  ...
  const isPreview = hostname.includes("run.app") || hostname.includes("webcontainer.io") ||
                    hostname.includes("stackblitz") || hostname.includes("codesandbox");
  // stea.africa, www.stea.africa, localhost, pages.dev are all treated as production
  ...
  const doGoogle = async () => {
    ...
    try {
      ...
      const res = await signInWithGoogleSafe(auth);
      if (res) {
        // Popup result (desktop)
        await saveUser(res.user, res.user.displayName, "google");
        onClose();
      } else {
        // Redirect initiated (mobile) — page will reload, result handled in useEffect
        // Don't call onClose() — the redirect is in progress
      }
    } catch (e) {
  ...
  const doEmail = async () => {
    ...
    try {
      console.log(`Attempting ${mode} for: ${normalizedEmail}`);
      if (mode === "login") {
...
function DigitalToolsPage({ goPage }) {
  ...
  if (!dealsLoading && dealsDocs.length > 0 && deals.length === 0) {
    console.log("[empty response reason] deals: All documents are marked as inactive (active: false).");
  }
          ...
          {coursesLoading ? (
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              <EmptyState 
                ...
                actionText="Angalia Tools"
                onAction={() => goPage("digital-tools")}
              />
            </div>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'rgba(255,255,255,0.02)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>
              {collection === 'gigs' ? "Hakuna kazi zilizopo kwa sasa." : `Hakuna ${hi} bado`}
            </h3>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Enable offline persistence so posts don't disappear on reconnect
    enableOfflinePersistence().catch(() => {});

    // Handle Google redirect sign-in result (mobile devices use redirect not popup)
    getRedirectResult(getFirebaseAuth()).then(async (result) => {
      if (result?.user) {
        console.log("[auth] Redirect result user:", result.user.email);
        // saveUser is called by onAuthStateChanged below automatically
      }
    }).catch((e) => {
      if (e.code !== "auth/no-current-user") {
        console.error("[auth] Redirect result error:", e.message);
      }
    });

    const unsubAuth = onAuthStateChanged(getFirebaseAuth(), async (u) => {
      console.log("[auth state] User changed:", u ? `${u.email} (${u.uid})` : "Logged out");
      if (u) {
        const db = getFirebaseDb();
        ...
        if (db) {
          try {
            ...
            if (s.exists()) {
              const data = s.data();
              // Only apply Firestore role if user is NOT already admin by email
        ...
        console.log("Resolved role:", role, "Source:", source);
    ...
    <>
      ...
      {adminOpen ? (
        ...
        >
          ...
          <main style={{ paddingBottom: isMobile ? "calc(72px + env(safe-area-inset-bottom, 0px))" : 0 }}>
            <Routes>
              ...
              <Route path="/course-detail" element={<CourseDetailPage course={selectedCourse} goPage={goPage} />} />
              ...
              <Route path="/news" element={<Navigate to="/tips" replace />} />
