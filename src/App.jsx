...
function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("stea-install-dismissed")) return;
    if (window.matchMedia("(display-mode: standalone)").matches) return;

    const handler = (e) => {
      e.preventDefault();
      setDeferredPrompt(e);
      setTimeout(() => setShow(true), 4000); // show after 4s
    };
  ...
  const dismiss = () => {
    setShow(false);
    localStorage.setItem("stea-install-dismissed", "1");
  };
  ...
  return (
    <div style={{
      ...
      zIndex: 8000, width: "min(380px, calc(100vw - 32px))",
      background: "rgba(14,16,26,0.97)", border: "1px solid rgba(245,166,35,0.3)",
      borderRadius: 20, padding: "16px 18px", display: "flex", alignItems: "center",
      gap: 14, boxShadow: "0 16px 48px rgba(0,0,0,0.6), 0 0 0 1px rgba(245,166,35,0.08)",
      backdropFilter: "blur(20px)", animation: "slideUpIn 0.4s cubic-bezier(0.34,1.56,0.64,1)"
    }}>
      <img src="/stea-icon.png" alt="STEA" style={{ width: 44, height: 44, objectFit: "contain", flexShrink: 0 }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 2 }}>Install STEA App</div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", lineHeight: 1.4 }}>Pata STEA kama app kwenye simu yako. Haraka zaidi!</div>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
        <button onClick={handleInstall} style={{
          border: "none", borderRadius: 10, padding: "8px 14px", fontWeight: 900,
          fontSize: 12, cursor: "pointer", background: `linear-gradient(135deg,${G},${G2})`, color: "#111"
        }}>Install</button>
        <button onClick={dismiss} style={{
          border: "none", borderRadius: 10, padding: "6px 14px", fontWeight: 700,
          fontSize: 11, cursor: "pointer", background: "rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.5)"
        }}>Baadaye</button>
      </div>
    </div>
  );
...
function NotificationBanner() {
  ...
  return (
    <div style={{
      ...
      background: "rgba(14,16,26,0.97)", border: "1px solid rgba(255,255,255,0.1)",
      borderRadius: 18, padding: "16px 16px", display: "flex", gap: 12,
      boxShadow: "0 16px 48px rgba(0,0,0,0.55)", backdropFilter: "blur(20px)",
      animation: "slideInRight 0.4s cubic-bezier(0.34,1.56,0.64,1)"
    }}>
      <div style={{ fontSize: 26, flexShrink: 0 }}>🔔</div>
      <div style={{ flex: 1 }}>
        {done ? (
          <div style={{ fontWeight: 800, color: G, fontSize: 14 }}>✅ Umesub notifications!</div>
        ) : (
          <>
            <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 4 }}>Pata notifications za STEA</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.48)", lineHeight: 1.5, marginBottom: 12 }}>
              Pokea habari mpya, tech tips na deals mara zinapotoka.
            </div>
      ...
      <button onClick={dismiss} style={{
        ...
        cursor: "pointer", fontSize: 16, display: "grid", placeItems: "center", borderRadius: 6
      }}>✕</button>
    </div>
  );
}
...
function WebsitesPage(){
  ...
  return(<section style={{padding:"26px 0"}}><W>
    <SHead title="Websites za Siri" hi="🤫" copy="Gundua websites za siri ambazo zinaweza kubadilisha namna unavyotumia internet kila siku. Kuanzia kuangalia movies bure 🎬, kujifunza lugha 🌍, kutumia AI tools 🛠, hadi kupata tools za kukusaidia kutatua matatizo mbalimbali 🔍 — hapa utapata gems ambazo watu wengi hawazijui. Jaribu moja tu, utashangaa."/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:22}}>
...
function HomePage({goPage}){
  ...
  return(<section style={{padding:"22px 0 16px"}}><W>
    ...
    <div style={{marginTop:40,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:16}}>
      {[{t:"Tech Tips",d:"Maujanja ya kila siku"},{t:"Tech Updates",d:"Habari za tech duniani"},{t:"Online Courses",d:"Jifunze skills za tech"},{t:"Deals",d:"Ofa na punguzo kali"},{t:"Prompt Lab",d:"AI prompts za nguvu"},{t:"Duka",d:"Bidhaa na huduma"}].map((h,i)=>(
    ...
    <div style={{marginTop:80, padding: 40, borderRadius: 32, border: "1px solid rgba(255,255,255,.05)", background: "rgba(255,255,255,.02)", textAlign: "center"}}>
        <h2 style={{fontFamily:"'Bricolage Grotesque',sans-serif", fontSize: 32, marginBottom: 16}}>Websites za Siri 🤫</h2>
        <p style={{color: "rgba(255,255,255,.6)", marginBottom: 24}}>Gundua websites ambazo watu wengi hawazijui…</p>
