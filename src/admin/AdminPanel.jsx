  ...
  const save = async () => {
    ...
    try {
      const data = { 
        ...
        sectionType: "techTips",
        views: form.views || 0, 
        createdAt: editing ? form.createdAt : serverTimestamp(),
...
export default function AdminPanel({ user, onBack }) {
  ...
  useEffect(() => {
    if (!db) return;
    const cols = ["tips","deals","courses","users","products","websites","prompts", "sponsored_ads", "orders", "subscriptions", "payments", "deliveries", "message_templates"];
    ...
    <div className="admin-panel-container" style={{ minHeight:"100vh", display:"grid", gridTemplateColumns: "240px 1fr", background:"#0a0b0f" }}>
      <style>{`
        @media (max-width: 768px) {
          ...
          .admin-sidebar { display: ${sidebarOpen ? 'block' : 'none'} !important; position: fixed !important; z-index: 1000 !important; background: #0a0b0f !important; width: 100% !important; }
        }
      ...
      <div className="admin-sidebar" style={{ borderRight:"1px solid rgba(255,255,255,.06)", padding:"24px 16px", position:"sticky", top:0, height:"100vh", overflowY:"auto" }}>
        <div style={{ marginBottom:28, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', color: '#fff', fontSize: 24, cursor: 'pointer' }}>×</button>
        </div>
      ...
      <div style={{ padding:"28px 32px", overflowY:"auto" }}>
        <button onClick={() => setSidebarOpen(true)} style={{ marginBottom: 20, padding: '10px 15px', background: G, border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>☰ Menu</button>
            ...
            <div style={{ borderRadius:20, border:"1px solid rgba(245,166,35,.2)", background:"rgba(245,166,35,.06)", padding:24 }}>
              ...
              <div style={{ display:"grid", gap:12 }}>
                {[
                  ...
                  { step:"2", title:"Update Deals na links za kweli", desc:"Nenda Deals → badilisha URL za dummy na affiliate links zako za kweli" },
