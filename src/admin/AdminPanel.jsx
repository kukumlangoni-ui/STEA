...
function ImageUploadField({ label, value, onChange }) {
  ...
  const hasPreview = (isDataUrl || isUrl) && !previewErr;

  // Reset preview error when URL changes
  const handleChange = (v) => { setPreviewErr(false); onChange(v); };
  ...
  return (
    <Field label={label}>
      {editing && isDataUrl && (
        <ImageEditor
          image={value}
          onSave={(cropped) => { onChange(cropped); setEditing(false); }}
          onCancel={() => setEditing(false)}
        />
      )}

      {/* Input row */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, marginBottom: hasPreview ? 10 : 0 }}>
        {isDataUrl ? (
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 12, padding: "0 10px", height: 46 }}>
            <div style={{ width: 32, height: 32, borderRadius: 6, overflow: "hidden", flexShrink: 0 }}>
              <img src={value} style={{ width: "100%", height: "100%", objectFit: "cover" }} referrerPolicy="no-referrer" onError={(e) => { e.target.style.display="none"; }} />
            </div>
            ...
            <button onClick={() => onChange("")} style={{ background: "transparent", border: "none", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: 18, lineHeight: 1 }}>×</button>
          </div>
        ) : (
          <div style={{ position: "relative" }}>
            ...
            {value && (
              <button
                ...
                style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "transparent", border: "none", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: 18, lineHeight: 1, padding: 0 }}
              >×</button>
            )}
          </div>
        )}
        <Btn onClick={() => fileInputRef.current.click()} color="rgba(255,255,255,.08)" textColor="#fff" style={{ padding: "0 14px", whiteSpace: "nowrap" }} disabled={loading}>
          {loading ? "⏳" : "📸 Upload"}
        </Btn>
        <input type="file" ref={fileInputRef} onChange={onFileChange} accept="image/*" style={{ display: "none" }} />
      </div>

      {/* Enlarged preview box — shown for both URL and uploaded images */}
      {hasPreview && (
        <div style={{ borderRadius: 12, overflow: "hidden", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", position: "relative", height: 160 }}>
          <img
            src={value}
            alt="Preview"
          ...
          <div style={{ position: "absolute", top: 8, right: 8 }}>
            <span style={{ fontSize: 10, fontWeight: 800, padding: "3px 8px", borderRadius: 6, background: "rgba(0,0,0,.7)", color: "rgba(255,255,255,.7)", letterSpacing: ".04em" }}>PREVIEW</span>
          </div>
        </div>
      )}
      {value && previewErr && (
        <div style={{ fontSize: 12, color: "#f87171", marginTop: 6, display: "flex", alignItems: "center", gap: 6 }}>
          ⚠️ Picha haiwezi kupakia — angalia URL au tumia upload
        </div>
      )}
    </Field>
  );
  ...
  const save = async () => {
    ...
    try {
      ...
      if (contentToStore.length > 500000) {
        ...
        contentToStore = ""; // Clear content in Firestore
      }
      ...
      const data = { 
        ...
        // Store imageUrl in BOTH fields so all UI components find it
      ...
      // Clean undefined/null values before sending to Firestore
      Object.keys(data).forEach(key => {
        if (data[key] === undefined || data[key] === null) data[key] = "";
      });

      // Block oversized base64 images — Firestore doc limit is ~1MB
      const imgVal = data.imageUrl || data.image || "";
      if (imgVal.startsWith("data:") && imgVal.length > 800_000) {
        throw new Error("Picha ni kubwa mno (zaidi ya 800KB). Tafadhali punguza ukubwa wa picha au tumia URL badala yake.");
      }
      ...
      if (editing) {
        ...
        setDocs(prev => prev.map(d => d.id === editing ? { ...d, ...updateData } : d));
        toast_("Imesahihishwa!");
      }
      else { 
        const docRef = await addDoc(collection(db, collectionName), data); 
        setDocs(prev => [{ ...data, id: docRef.id }, ...prev]);
...
function DigitalToolsManager() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        const docRef = await addDoc(collection(db, "deals"), data);
        setDocs(prev => [{ ...data, id: docRef.id }, ...prev]);
  ...
  const del = async (id) => {
    setConfirm({
      msg: "Una uhakika unataka kufuta tool hii? Hatua hii haiwezi kurejeshwa.",
      onConfirm: async () => {
        try {
          await deleteDoc(doc(db, "deals", id));
          setConfirm(null);
          toast_("Tool imefutwa");
        } catch (e) {
          toast_(e.message, "error");
        }
      },
  ...
  return (
    <div>
      ...
      <div style={{ borderRadius:20, border:"1px solid rgba(255,255,255,.08)", background:"#141823", padding:24, marginBottom:28 }}>
        <h3 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:20, margin:"0 0 20px" }}>
          {editing?"✏️ Hariri Tool":"➕ Ongeza Tool Mpya"}
        </h3>
        <div style={{ display:"grid", gap:16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            <Field label="Jina la Tool *"><Input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Canva Pro"/></Field>
          ...
          <Field label="Short Intro"><Textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Maelezo mafupi ya tool hii..." style={{minHeight:60}}/></Field>
          <Field label="Full Description"><Textarea value={form.fullDescription} onChange={e=>setForm(f=>({...f,fullDescription:e.target.value}))} placeholder="Maelezo kamili ya tool hii..." style={{minHeight:100}}/></Field>
          
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            <Field label="Why This Tool?"><Textarea value={form.whyThisDeal} onChange={e=>setForm(f=>({...f,whyThisDeal:e.target.value}))} placeholder="Kwa nini ununue tool hii?" style={{minHeight:80}}/></Field>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            <Field label="CTA Button Text"><Input value={form.ctaText} onChange={e=>setForm(f=>({...f,ctaText:e.target.value}))} placeholder="Pata Tool"/></Field>
          ...
          <Field label="Short Review/Testimonial"><Textarea value={form.reviewText} onChange={e=>setForm(f=>({...f,reviewText:e.target.value}))} placeholder="This tool saved me $100! - John" style={{minHeight:60}}/></Field>
      ...
      <div style={{ display:"grid", gap:12 }}>
        {docs.length===0 && <div style={{ textAlign:"center", padding:40, color:"rgba(255,255,255,.35)" }}>Hakuna tools bado. Ongeza ya kwanza! 👆</div>}
        {docs.filter(d => (d.name||"").toLowerCase().includes(search.toLowerCase())).map(item=>(
          <div key={item.id} style={{ borderRadius:16, border:`1px solid ${item.active?"rgba(255,255,255,.07)":"rgba(239,68,68,.2)"}`, background:item.active?"#1a1d2e":"rgba(239,68,68,.05)", padding:"14px 18px", display:"flex", gap:12, alignItems:"center", flexWrap:"wrap", maxWidth: 800 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.domain} · {item.code?"Code: "+item.code:"Referral"}</div>
            </div>
            ...
            <div style={{ width: '100%' }}>
              <SocialPoster content={item} type="Tool" />
            </div>
          </div>
        ))}
      </div>
...
function CoursesManager() {
  const [docs, setDocs] = useState([]);
  const [form, setForm] = useState({ 
    imageUrl:"", carouselImages: [], title:"", desc:"", free:true, price:"Bure · Start now", cta:"Anza Sasa →", lessons:"", adminWhatsAppNumber:"255768260933", accent:"",
      ...
      <div style={{ display:"grid", gap:12 }}>
        ...
        {docs.map(item=>(
          <div key={item.id} style={{ borderRadius:16, border:"1px solid rgba(255,255,255,.07)", background:"#1a1d2e", padding:"14px 18px", display:"flex", gap:12, alignItems:"center" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom:2 }}>{item.title}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.free?"🆓 Bure":"⭐ Paid"} · {item.price} · {(item.lessons||[]).length} lessons</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <Btn onClick={()=>{
                setEditing(item.id);
            ...
            <div style={{ width: '100%' }}>
              <SocialPoster content={item} type="Course" />
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        ...
        {docs.map(item => (
          <div key={item.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.category} · {item.price}</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <Btn onClick={() => { setEditing(item.id); setForm({ ...item }); window.scrollTo({ top: 0, behavior: "smooth" }); }} color="rgba(245,166,35,.12)" textColor={G} style={{ padding: "8px 14px" }}>✏️</Btn>
              <Btn onClick={() => del(item.id)} color="rgba(239,68,68,.12)" textColor="#fca5a5" style={{ padding: "8px 14px" }}>🗑️</Btn>
...
function WebsitesManager() {
  const [docs, setDocs] = useState([]);
  const [search, setSearch] = useState("");
  const [form, setForm] = useState({ name: "", url: "", description: "", iconUrl: "", imageUrl: "", bg: "linear-gradient(135deg,#667eea,#764ba2)", meta: "Free Tool", tags: "", category: "Free Movie" });
  const [editing, setEditing] = useState(null);
  const [loading, setLoading] = useState(false);
  const [toast, setToast] = useState(null);
  const [confirm, setConfirm] = useState(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm({ name: "", url: "", description: "", iconUrl: "", imageUrl: "", bg: "linear-gradient(135deg,#667eea,#764ba2)", meta: "Free Tool", tags: "", category: "Free Movie" });
      setEditing(null);
    } catch (e) {
      console.error(e);
      if (e.message.includes("insufficient permissions")) {
  ...
  return (
    <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            ...
            <Field label="Category *">
              <Select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}>
                {WEBSITE_CATEGORIES.map(c => <option key={c} value={c} style={{ background: "#1a1d2e" }}>{c}</option>)}
              </Select>
            </Field>
          </div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "rgba(255,255,255,.35)" }}>{search ? "Hakuna website inayolingana na utafutaji wako." : "Hakuna websites bado. Ongeza ya kwanza! 👆"}</div>}
        {filtered.map(item => (
          <div key={item.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            <AdminThumb url={item.iconUrl || item.imageUrl} fallback="🛠️" />
            <div style={{ flex: 1, minWidth: 0, maxWidth: 400 }}>
              <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.url}</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <Btn onClick={() => { setEditing(item.id); setForm({ iconUrl: "", imageUrl: "", category: "Free Movie", ...item, tags: (item.tags || []).join(", ") }); window.scrollTo({ top: 0, behavior: "smooth" }); }} color="rgba(245,166,35,.12)" textColor={G} style={{ padding: "8px 14px" }}>✏️</Btn>
              <Btn onClick={() => del(item.id)} color="rgba(239,68,68,.12)" textColor="#fca5a5" style={{ padding: "8px 14px" }}>🗑️</Btn>
            </div>
            <div style={{ width: '100%' }}>
              <SocialPoster content={item} type="Website" />
            </div>
          </div>
        ))}
      </div>
...
function PromptsManager() {
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...form,
        title: form.title || "",
        description: form.prompt || "", // Mapping prompt to description
        image: form.imageUrl || "", // Mapping imageUrl to image
        category: form.category || "General",
        active: form.active ?? true,
        published: form.published ?? true,
      ...
      if (editing) {
        ...
        delete updateData.id;
        await updateDoc(doc(db, "prompts", editing), updateData);
        setDocs(prev => prev.map(d => d.id === editing ? { ...d, ...updateData } : d));
        toast_("Prompt imebadilishwa");
      } else {
        const docRef = await addDoc(collection(db, "prompts"), payload);
        setDocs(prev => [{ ...payload, id: docRef.id }, ...prev]);
  ...
  return (
    <div>
          ...
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {form.tools.map((tool, i) => (
                <div key={i} style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "end", background: "rgba(255,255,255,.02)", padding: 12, borderRadius: 12 }}>
                  ...
                  <Btn onClick={() => removeTool(i)} color="rgba(239,68,68,.1)" textColor="#fca5a5" style={{ padding: 10, marginBottom: 16 }}>✕</Btn>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "rgba(255,255,255,.35)" }}>{search ? "Hakuna prompt inayolingana na utafutaji wako." : "Hakuna prompts bado. Ongeza ya kwanza! 👆"}</div>}
        {filtered.map(item => (
          <div key={item.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            <AdminThumb url={item.imageUrl} fallback="🤖" />
            <div style={{ flex: 1, minWidth: 0, maxWidth: 400 }}>
              <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.title}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.category} • {item.tools?.length || 0} Tools</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ width: '100%' }}>
              <SocialPoster content={item} type="Prompt" />
            </div>
          </div>
        ))}
      </div>
...
function FAQManager() {
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {faqs.map(f => (
          <div key={f.id} style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,.05)", background: "rgba(255,255,255,.02)", padding: 16, display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, fontSize: 14, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.question}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.category} • {f.isActive ? "Active" : "Inactive"}</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {filtered.map(u => (
              <div key={u.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: 2 }}>{u.name || "No name"}</div>
                  <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', wordBreak: "break-all" }}>{u.email}</div>
...
function SponsoredAdsManager() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (size > 950000) {
        throw new Error("Data ni kubwa mno (zidi 950KB). Tafadhali punguza picha au maelezo.");
      }
      
      if (editing) {
        const updateData = { ...data };
        delete updateData.id;
        await updateDoc(doc(db, "sponsored_ads", editing), updateData);
        setDocs(prev => prev.map(d => d.id === editing ? { ...d, ...updateData } : d));
        toast_("Imesahihishwa!");
      } else {
        const docRef = await addDoc(collection(db, "sponsored_ads"), data);
        setDocs(prev => [{ ...data, id: docRef.id }, ...prev]);
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {filtered.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "rgba(255,255,255,.35)" }}>{search ? "Hakuna ad inayolingana na utafutaji wako." : "Hakuna ads bado."}</div>}
        {filtered.map((item, index) => (
          <div key={`${item.id}-${index}`} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            <div style={{ flex: 1, minWidth: 0, maxWidth: 400 }}>
              <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.title} ({item.clientName})</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.adType} • {item.status}</div>
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <Btn onClick={() => { setEditing(item.id); setForm(item); window.scrollTo({ top: 0, behavior: "smooth" }); }} color="rgba(245,166,35,.12)" textColor={G} style={{ padding: "8px 14px" }}>✏️</Btn>
              <Btn onClick={() => del(item.id)} color="rgba(239,68,68,.12)" textColor="#fca5a5" style={{ padding: "8px 14px" }}>🗑️</Btn>
            ...
            <div style={{ width: '100%' }}>
              <SocialPoster content={item} type="Sponsored Ad" />
            </div>
          </div>
        ))}
      </div>
...
function SubscriptionManager() {
  ...
  const sendReminder = async (sub) => {
    // Logic to send reminder (e.g., update a 'reminders' collection or trigger a cloud function)
        ...
        {subs.map(s => (
          <div key={s.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Ends: {s.endDate?.toDate().toLocaleDateString()}</div>
            </div>
...
function MessageTemplateManager() {
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await updateDoc(doc(db, "message_templates", editing), { ...form, updatedAt: serverTimestamp() });
        toast_("Imesahihishwa!");
      } else {
        await addDoc(collection(db, "message_templates"), { ...form, createdAt: serverTimestamp() });
        toast_("Imewekwa!");
      }
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {templates.map(t => (
          <div key={t.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.name}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.content.substring(0, 50)}...</div>
            </div>
...
export default function AdminPanel({ user, onBack }) {
  ...
  const [counts,  setCounts]  = useState({ tips:0, deals:0, courses:0, users:0, products:0, websites:0, prompts:0, sponsored_ads:0, orders:0, subscriptions:0, payments:0, deliveries:0, message_templates:0 });
  const [toast, setToast] = useState(null);
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    if (!db) return;
    const cols = ["tips","deals","courses","users","marketplace","websites","prompts", "sponsored_ads", "orders", "subscriptions", "payments", "deliveries", "message_templates", "necta", "exams", "news", "ai", "gigs"];
  ...
  const SECTIONS = [
    ...
    { id:"updates",   icon:"📰", label:"Updates/Posts" },
  ...
  return (
    <div className="admin-panel-container" style={{ minHeight:"100vh", display:"grid", gridTemplateColumns: "240px 1fr", background:"#0a0b0f" }}>
      ...
      <div className="admin-main-content" style={{ padding:"28px 32px", overflowY:"auto", minWidth: 0 }}>
        ...
        {section==="overview" && (
          <div>
            ...
            <div style={{ marginBottom:28, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 20 }}>
              <div>
                ...
                <p style={{ color:"rgba(255,255,255,.45)", fontSize:15, margin:0 }}>
                  Hapa unaweza kumanage content yote ya STEA — posts, tools, courses na users.
                </p>
              </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:16, marginBottom:32 }}>
              ...
              <StatCard icon="🏷️" label="Digital Tools" value={counts.deals} color="#a5b4fc"/>
              <StatCard icon="🎓" label="Courses" value={counts.courses} color="#67f0c1"/>
              <StatCard icon="🛒" label="Duka Products" value={counts.marketplace} color="#fbbf24"/>
              <StatCard icon="🌐" label="Websites" value={counts.websites} color="#818cf8"/>
              <StatCard icon="👥" label="Users" value={counts.users} color="#ff85cf"/>
              <StatCard icon="🎓" label="NECTA" value={counts.necta} color="#67f0c1"/>
              <StatCard icon="📝" label="Exams" value={counts.exams} color="#a5b4fc"/>
              <StatCard icon="💼" label="Gigs" value={counts.gigs} color="#818cf8"/>
              <StatCard icon="🤖" label="AI Lab" value={counts.ai} color="#ff85cf"/>
              ...
              <StatCard icon="✉️" label="Templates" value={counts.message_templates} color="#fbbf24"/>
            </div>
        ...
        {section==="marketplace" && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🛒 <span style={{color:G}}>STEA Duka (Marketplace)</span></h2><MarketplaceManager/></>}
        {section==="vpn"      && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🛡️ Manage <span style={{color:G}}>VPN & Payments</span></h2><VpnManager/></>}
        {section==="examshub" && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>📚 Manage <span style={{color:G}}>Exams Hub</span></h2><ExamsHubManager/></>}
        {section==="necta"    && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🎓 Manage <span style={{color:G}}>NECTA Results</span></h2><TechContentManager collectionName="necta" /></>}
        {section==="exams"    && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>📝 Manage <span style={{color:G}}>Old Exams</span></h2><TechContentManager collectionName="exams" /></>}
        {section==="ai"       && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🤖 Manage <span style={{color:G}}>AI Lab</span></h2><TechContentManager collectionName="ai" /></>}
        {section==="gigs"     && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>💼 Manage <span style={{color:G}}>Gigs & Jobs</span></h2><TechContentManager collectionName="gigs" /></>}
        ...
        {section==="updates"  && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>📰 Manage <span style={{color:G}}>Updates & Posts</span></h2><TechContentManager collectionName="updates" /></>}
        ...
        {section==="deals"   && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🏷️ Manage <span style={{color:G}}>Digital Tools</span></h2><DigitalToolsManager/></>}
        ...
        {section==="products" && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🛒 Manage <span style={{color:G}}>Legacy Products</span></h2><ProductsManager/></>}
