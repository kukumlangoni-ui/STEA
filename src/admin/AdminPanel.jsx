...
import { onSnapshot, increment } from "firebase/firestore";
...
const ImageUpload = ({ label, value, onChange }) => (
  <div style={{ display:"grid", gap:6 }}>
    ...
    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
      {value && <img src={value} alt="Preview" style={{ width:60, height:46, objectFit:"cover", borderRadius:8, border:"1px solid rgba(255,255,255,.1)" }} />}
      <input type="file" accept="image/*" onChange={async (e) => {
        if(e.target.files && e.target.files[0]) {
          const base64 = await compressImage(e.target.files[0]);
          onChange(base64);
        }
      }} style={{ flex:1, height:46, borderRadius:12, border:"1px solid rgba(255,255,255,.1)", background:"rgba(255,255,255,.05)", color:"#fff", padding:"9px 14px", outline:"none", fontSize:14, cursor:"pointer" }} />
    </div>
  </div>
);

import { getVideoThumb, ArticleModal, VideoModal } from '../App';
...
function TipsManager() {
  ...
  const [form, setForm] = useState({ type:"article", badge:"Android", title:"", summary:"", content:"", coverImage:"", tags:"", readTime:"5 min", platform:"youtube", embedUrl:"", channel:"", channelImg:"🎙️", duration:"" });
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm({ type:"article", badge:"Android", title:"", summary:"", content:"", coverImage:"", tags:"", readTime:"5 min", platform:"youtube", embedUrl:"", channel:"", channelImg:"🎙️", duration:"" });
  ...
  return (
    <div>
      {previewArticle && <ArticleModal article={previewArticle} onClose={()=>setPreviewArticle(null)}/>}
      {previewVideo && <VideoModal video={previewVideo} onClose={()=>setPreviewVideo(null)}/>}
      ...
      <div style={{ borderRadius:20, border:"1px solid rgba(255,255,255,.08)", background:"#141823", padding:24, marginBottom:28 }}>
        ...
        <div style={{ display:"grid", gap:16 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            <Field label="Badge (Android / iPhone / AI / PC)">
            ...
            <ImageUpload label="Cover Image (Thumbnail) *" value={form.coverImage} onChange={val=>setForm(f=>({...f,coverImage:val}))} />
          </div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            {editing && <Btn onClick={()=>{setEditing(null);setForm({type:"article",badge:"Android",title:"",summary:"",content:"",coverImage:"",tags:"",readTime:"5 min",platform:"youtube",embedUrl:"",channel:"",channelImg:"🎙️",duration:""});}} color="rgba(255,255,255,.08)" textColor="#fff">✕ Acha</Btn>}
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gap:12 }}>
        ...
        {docs.map(item=>(
          <div key={item.id} style={{ borderRadius:16, border:"1px solid rgba(255,255,255,.07)", background:"#1a1d2e", padding:"16px 20px", display:"flex", gap:14, alignItems:"center", flexWrap:"wrap" }}>
            <div style={{ width:60, height:60, borderRadius:10, background:"#000", overflow:"hidden", flexShrink:0 }}>
              <img src={item.type === "video" ? getVideoThumb(item) : (item.coverImage||"https://images.unsplash.com/photo-1618401471353-b98afee0b2eb?q=80&w=200&auto=format&fit=crop")} alt="thumb" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            </div>
            ...
            <div style={{ display:"flex", gap:8, flexShrink:0 }}>
              <Btn onClick={()=>{if(item.type==="video")setPreviewVideo(item);else setPreviewArticle(item);}} color="rgba(59,130,246,.12)" textColor="#93c5fd" style={{padding:"8px 14px"}}>👁️</Btn>
...
function UpdatesManager() {
  const [docs, setDocs] = useState([]);
  const [form, setForm] = useState({ type:"article", badge:"AI", category:"Artificial Intelligence", title:"", summary:"", content:"", coverImage:"", source:"", platform:"youtube", embedUrl:"", channel:"", channelImg:"🔥", duration:"" });
  const [editing, setEditing] = useState(null);
  const [loading, setLoading] = useState(false);
  const [toast,   setToast]   = useState(null);
  const [confirm, setConfirm] = useState(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm({ type:"article", badge:"AI", category:"Artificial Intelligence", title:"", summary:"", content:"", coverImage:"", source:"", platform:"youtube", embedUrl:"", channel:"", channelImg:"🔥", duration:"" });
      setEditing(null); loadDocs();
    } catch(e) { toast_(e.message,"error"); }
    setLoading(false);
  };
  ...
  const del = async (id) => {
    setConfirm({
      msg: "Una uhakika unataka kufuta habari hii?",
      onConfirm: async () => {
        try {
      ...
      <div style={{ borderRadius:20, border:"1px solid rgba(255,255,255,.08)", background:"#141823", padding:24, marginBottom:28 }}>
        ...
        <div style={{ display:"grid", gap:16 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:16 }}>
            <Field label="Badge (AI / Android / iPhone)">
              <Select value={form.badge} onChange={e=>setForm(f=>({...f,badge:e.target.value}))}>
            ...
            <ImageUpload label="Cover Image *" value={form.coverImage} onChange={val=>setForm(f=>({...f,coverImage:val}))} />
          </div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            {editing && <Btn onClick={()=>{setEditing(null);setForm({type:"article",badge:"AI",category:"Artificial Intelligence",title:"",summary:"",content:"",coverImage:"",source:"",platform:"youtube",embedUrl:"",channel:"",channelImg:"🔥",duration:""});}} color="rgba(255,255,255,.08)" textColor="#fff">✕ Acha</Btn>}
          </div>
        </div>
      </div>
...
function ProductsManager() {
  const [docs, setDocs] = useState([]);
  const [form, setForm] = useState({ name: "", description: "", price: "", oldPrice: "", icon: "🎧", badge: "", url: "", category: "Electronics", coverImage: "" });
  const [editing, setEditing] = useState(null);
  const [loading, setLoading] = useState(false);
  const [toast, setToast] = useState(null);
  const [confirm, setConfirm] = useState(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm({ name: "", description: "", price: "", oldPrice: "", icon: "🎧", badge: "", url: "", category: "Electronics", coverImage: "" });
      setEditing(null); loadDocs();
    } catch (e) { toast_(e.message, "error"); }
    setLoading(false);
  };
    ...
    <div>
      ...
      <div style={{ borderRadius: 20, border: "1px solid rgba(255,255,255,.08)", background: "#141823", padding: 24, marginBottom: 28 }}>
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <ImageUpload label="Cover Image (Optional)" value={form.coverImage} onChange={val=>setForm(f=>({...f,coverImage:val}))} />
      ...
      <div style={{ display: "grid", gap: 12 }}>
        ...
        {docs.map(item => (
          <div key={item.id} style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,.07)", background: "#1a1d2e", padding: "14px 18px", display: "flex", gap: 12, alignItems: "center" }}>
            {item.coverImage ? <img src={item.coverImage} alt="thumb" style={{width:40,height:40,objectFit:"cover",borderRadius:8}}/> : <div style={{ fontSize: 32 }}>{item.icon}</div>}
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800, fontSize: 15 }}>{item.name}</div>
...
function ChatsManager() {
  ...
  const send = async (e) => {
    e.preventDefault();
    if (!msg.trim() || !activeChat || !db) return;
    const text = msg.trim();
    setMsg("");
    try {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", height: "calc(100vh - 120px)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 16, overflow: "hidden" }}>
      {/* Chat List */}
      <div style={{ borderRight: "1px solid rgba(255,255,255,.1)", background: "rgba(255,255,255,.02)", overflowY: "auto" }}>
        <div style={{ padding: 16, borderBottom: "1px solid rgba(255,255,255,.1)", fontWeight: 800 }}>Meseji za Wateja</div>
        {chats.length === 0 ? (
          <div style={{ padding: 20, textAlign: "center", color: "rgba(255,255,255,.4)", fontSize: 13 }}>Hakuna meseji.</div>
        ) : (
          chats.map(c => (
            <div key={c.id} onClick={() => setActiveChat(c)} style={{ padding: 16, borderBottom: "1px solid rgba(255,255,255,.05)", cursor: "pointer", background: activeChat?.id === c.id ? "rgba(245,166,35,.1)" : "transparent", display: "flex", gap: 12, alignItems: "center", transition: "background .2s" }}>
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,.1)", overflow: "hidden", flexShrink: 0 }}>
                {c.userPhoto ? <img src={c.userPhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "grid", placeItems: "center", fontSize: 20 }}>👤</div>}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.userName}</div>
                  {c.unreadAdmin > 0 && <div style={{ background: "#ef4444", color: "#fff", fontSize: 10, fontWeight: 900, padding: "2px 6px", borderRadius: 10 }}>{c.unreadAdmin}</div>}
                </div>
      ...
      {/* Chat Area */}
      {activeChat ? (
        <div style={{ display: "flex", flexDirection: "column", background: "#0c0e16" }}>
          <div style={{ padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,.1)", display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,.02)" }}>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(255,255,255,.1)", overflow: "hidden" }}>
              {activeChat.userPhoto ? <img src={activeChat.userPhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "grid", placeItems: "center", fontSize: 18 }}>👤</div>}
            </div>
            <div>
              <div style={{ fontWeight: 800 }}>{activeChat.userName}</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,.4)" }}>{activeChat.id}</div>
            </div>
          </div>

          <div style={{ flex: 1, overflowY: "auto", padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
            {messages.map(m => (
              <div key={m.id} style={{ alignSelf: m.sender === "admin" ? "flex-end" : "flex-start", maxWidth: "70%", background: m.sender === "admin" ? "rgba(245,166,35,.15)" : "rgba(255,255,255,.08)", border: m.sender === "admin" ? `1px solid rgba(245,166,35,.3)` : "1px solid rgba(255,255,255,.1)", padding: "12px 16px", borderRadius: 16, borderBottomRightRadius: m.sender === "admin" ? 4 : 16, borderBottomLeftRadius: m.sender === "user" ? 4 : 16, color: m.sender === "admin" ? G : "#fff", fontSize: 14, lineHeight: 1.5 }}>
                {m.text}
              </div>
            ))}
          ...
          <form onSubmit={send} style={{ padding: 16, borderTop: "1px solid rgba(255,255,255,.1)", display: "flex", gap: 12, background: "rgba(255,255,255,.02)" }}>
            <input value={msg} onChange={e => setMsg(e.target.value)} placeholder="Andika jibu lako hapa..." style={{ flex: 1, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 99, padding: "12px 20px", color: "#fff", fontSize: 14, outline: "none" }} />
            <button type="submit" disabled={!msg.trim()} style={{ width: 44, height: 44, borderRadius: "50%", background: msg.trim() ? `linear-gradient(135deg,${G},${G2})` : "rgba(255,255,255,.1)", color: msg.trim() ? "#111" : "rgba(255,255,255,.3)", border: "none", cursor: msg.trim() ? "pointer" : "not-allowed", display: "grid", placeItems: "center", fontSize: 18, fontWeight: 900 }}>↑</button>
          </form>
        </div>
      ) : (
        <div style={{ display: "grid", placeItems: "center", color: "rgba(255,255,255,.3)", fontSize: 14 }}>
          Chagua meseji upande wa kushoto ili kuanza kuchat.
        </div>
      )}
    </div>
  );
...
export default function AdminPanel({ user, onBack }) {
  ...
  useEffect(() => {
    ...
    const unsubChats = onSnapshot(collection(db, "chats"), (snap) => {
      let unread = 0;
      snap.forEach(d => { if (d.data().unreadAdmin > 0) unread++; });
  ...
  const SECTIONS = [
    ...
    { id:"chats",    icon:"💬", label:"Meseji", badge: unreadChats > 0 ? unreadChats : null },
  ...
  return (
    <div style={{ minHeight:"100vh", display:"grid", gridTemplateColumns:"240px 1fr", background:"#0a0b0f" }}>
      ...
      <div style={{ padding:"28px 32px", overflowY:"auto" }}>
        ...
        {section==="chats"   && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>💬 Manage <span style={{color:G}}>Meseji</span></h2><ChatsManager/></>}
