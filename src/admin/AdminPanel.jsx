...
// ── Stats Card ────────────────────────────────────────
function StatCard({ icon, label, value, color = G, error = null }) {
  return (
    <div style={{ borderRadius:18, border:"1px solid rgba(255,255,255,.08)", background:"#141823",
      ...
      <div>
        <div style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize: error ? 14 : 28, fontWeight:800,
          color: error ? "#ef4444" : color, lineHeight:1 }}>
          {error ? "Permission Denied" : value}
        </div>
...
function ImageUploadField({ label, value, onChange }) {
  ...
  const onFileChange = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (file.size > 700000) { // ~700KB limit to stay under 1MB after base64
        toast_("Picha hii ni kubwa mno (zidi 700KB). Tafadhali punguza size ya picha kwanza.", "error");
        return;
      }
      setLoading(true);
      ...
      reader.onload = () => {
        ...
        setLoading(false);
        toast_("Picha imepakiwa vizuri!");
      };
      reader.onerror = () => {
        setLoading(false);
        toast_("Imeshindwa kusoma picha", "error");
      };
  ...
  const handleSaveEditedImage = async (cropped) => {
    setLoading(true);
    try {
      console.log("ImageUploadField: handleSaveEditedImage started");
      await onChange(cropped);
      console.log("ImageUploadField: onChange completed");
      ...
      setLoading(false);
      console.log("ImageUploadField: handleSaveEditedImage finished");
    }
  };
    ...
    <Field label={label}>
      {editing && value && (
        <ImageEditor
          ...
          onCancel={() => {
            console.log("ImageUploadField: editing cancelled");
            setEditing(false);
          }}
        />
      )}
        ...
        {isDataUrl ? (
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            {value && (
              <img 
                src={value} 
                style={{ position: 'absolute', left: 8, width: 30, height: 30, borderRadius: 6, objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)' }} 
            ...
            {value && (
              <div style={{ position: 'absolute', right: 8, display: 'flex', gap: 4, alignItems: 'center' }}>
                <button 
                  ...
                  style={{ background: "rgba(245,166,35,.1)", border: "none", color: G, cursor: "pointer", fontSize: 12, padding: "4px 8px", borderRadius: 6, fontWeight: 700 }}
                  title="Crop Image"
                >
                  ✏️
                </button>
                <button 
                  onClick={() => onChange("")} 
                  style={{ background: "transparent", border: "none", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: 16 }}
                >
                  ✕
                </button>
              </div>
  ...
  return (
    <div className="admin-panel-container" style={{ minHeight:"100vh", display:"grid", gridTemplateColumns: "240px 1fr", background:"#0a0b0f" }}>
      ...
      <div className="admin-main-content" style={{ padding:"28px 32px", overflowY:"auto", minWidth: 0 }}>
        ...
        {section==="posts" && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>📝 Manage <span style={{color:G}}>All Posts</span></h2><TechContentManager collectionName="posts" /></>}
        {section==="updates" && <><h2 style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:28, margin:"0 0 24px" }}>🔔 Manage <span style={{color:G}}>Site Updates</span></h2><TechContentManager collectionName="updates" /></>}
