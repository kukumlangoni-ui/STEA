...
const FILTERS = [
  ...
  { id: 'heslb',   label: 'HESLB Updates',   emoji: '💳' },
  { id: 'tcu',     label: 'TCU Updates',      emoji: '🏛️' },
    ...
    >
      ...
      {item.link && (
        <a href={item.link} target="_blank" rel="noreferrer" style={{
          ...
          <ExternalLink size={12} /> Soma Zaidi
        </a>
      )}
    </motion.div>
  );
...
export default function StudentUpdatesPage() {
  ...
  // Try to load from Firestore (updates collection)
  const { docs: updatesDocs, loading } = useCollection('updates', 'createdAt', 30);
  const { docs: newsDocs } = useCollection('news', 'createdAt', 20);

  const allRemote = [...updatesDocs, ...newsDocs]
    .filter((item, idx, self) => self.findIndex(t => t.id === item.id) === idx)
    .sort((a, b) => {
      const getTs = d => d.createdAt?.seconds || 0;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0b0f', color: '#fff', paddingBottom: 100 }}>
        ...
        <W>
          <div style={{
            ...
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.6, margin: 0 }}>
              ...
              <button onClick={() => navigate('/exams/university-guide')} style={{ background: 'none', border: 'none', color: '#60a5fa', fontWeight: 800, cursor: 'pointer', fontSize: 13 }}>University Guide TZ</button>
              {'. '}
              Kwa scholarship, nenda{' '}
              <button onClick={() => navigate('/exams/scholarships')} style={{ background: 'none', border: 'none', color: '#10b981', fontWeight: 800, cursor: 'pointer', fontSize: 13 }}>Scholarships Tanzania</button>.
            </p>
          </div>
        </W>
      </section>
        ...
        <W>
          {loading ? (
            ...
            <div style={{
              ...
              <h3 style={{ fontWeight: 800, marginBottom: 8 }}>Hakuna habari za "{activeFilter}" kwa sasa</h3>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, marginBottom: 20 }}>Jaribu kichujio kingine au angalia zote.</p>
      ...
      <section style={{ padding: '0 0 clamp(48px,6vw,72px)' }}>
        <W>
          <div style={{ borderRadius: 20, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', padding: '24px 22px' }}>
            <h3 style={{ fontSize: 15, fontWeight: 800, marginBottom: 16, color: 'rgba(255,255,255,0.8)' }}>Viungo vya Haraka</h3>
