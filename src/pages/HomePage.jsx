...
const normalizeItem = (item) => ({
  ...
  image: item.imageUrl || item.image || (item.carouselImages && item.carouselImages[0]) || "",
...
export default function HomePage({ goPage }) {
  ...
  const { docs: tipsDocs,    loading: tipsLoading    } = useCollection("tips",    "createdAt", 10);
  ...
  const { docs: productsDocs, loading: productsLoading } = useCollectionWhere("products", "sector", "==", "marketplace", "createdAt", 20);

  // Loose filters to support legacy documents and ensure quality
  const filterFn = (item) => {
    // For debugging: let's allow EVERYTHING to see if data appears
    // We will only exclude explicit drafts if we really want to, but for now let's just return true
    // to prove the data is reaching the component.
    if (item.status === "draft") return false;
    return true;
  };

  const tips = tipsDocs.filter(filterFn).slice(0, 6);
  const courses = coursesDocs.filter(filterFn).slice(0, 4);
          ...
          <W>
            <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center", paddingTop: isMobile ? "100px" : "140px", paddingBottom: isMobile ? "80px" : "120px" }}>
              ...
              <div style={{ marginTop: isMobile ? 64 : 80, display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: isMobile ? 16 : 24 }}>
                ...
                <HeroServiceCard title={t('pillar_duka_title')} icon="🛒" desc={t('pillar_duka_tag')} onClick={() => goPage("duka")} />
              </div>
            </div>
          </W>
        </motion.div>
      ...
      <section style={{ padding: "40px 0" }}>
        <W>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2,1fr)", gap: 20 }}>
            ...
            <PillarCard emoji="🛒" title={t('pillar_duka_title')} tag={t('pillar_duka_tag')} color="#f5a623" path="duka" desc={t('pillar_duka_desc')} goPage={goPage} delay={.24} />
          </div>
        </W>
      </section>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 24 }}>
            <div style={{ background: "rgba(255,255,255,.02)", borderRadius: 24, padding: "32px", border: "1px solid rgba(255,255,255,.06)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                ...
                <button onClick={() => goPage("duka")} style={{ background: "none", border: "none", color: G, fontWeight: 800, cursor: "pointer" }}>{t('action_view_all')}</button>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
                {products.slice(0, 4).map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => goPage("duka")} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, overflow: "hidden", cursor: "pointer" }}>
              ...
              <GoldBtn onClick={() => goPage("duka")} style={{ width: "100%", justifyContent: "center" }}>{t('action_explore')}</GoldBtn>
            </div>
