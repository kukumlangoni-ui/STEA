...
function HeroSection({ goPage, sectorsRef, deferredPrompt, installApp, isInstalled, t }) {
  ...
  const heroY  = useTransform(scrollYProgress, [0, 1], ["0px", "70px"]);
  ...
  return (
    <section ref={heroRef} style={{
      position: "relative", minHeight: isMobile ? "86vh" : "100vh",
      overflow: "hidden",
      background: "linear-gradient(180deg,#060a14 0%,#0c1124 100%)",
    }}>
      {/* Mesh gradients */}
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 2,
        background: "radial-gradient(ellipse 85% 55% at 12% 4%,rgba(245,166,35,.08) 0%,transparent 55%), radial-gradient(ellipse 65% 48% at 88% 12%,rgba(59,130,246,.07) 0%,transparent 50%), radial-gradient(ellipse 45% 38% at 50% 96%,rgba(139,92,246,.05) 0%,transparent 50%)"
      }} />
      {/* Globe */}
      <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none", zIndex: 1 }}>
        <AnimatedEarth />
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 55% 45%,transparent 0%,rgba(6,10,20,.52) 100%)", zIndex: 5 }} />
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 340,
          background: "linear-gradient(to top,rgba(6,10,20,.98) 0%,rgba(6,10,20,.7) 50%,transparent 100%)", zIndex: 6 }} />
      </div>
      ...
      <motion.div style={{ position: "relative", zIndex: 10, width: "100%", y: heroY, opacity: heroOp }}>
        <W>
          <div style={{ maxWidth: 860, margin: "0 auto", textAlign: "center",
            paddingTop: isMobile ? "clamp(100px,22vw,130px)" : "clamp(130px,14vw,170px)",
            paddingBottom: isMobile ? 72 : 100 }}>

            {/* Badge */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: SPRING }}>
              <div style={{
                display: "inline-flex", alignItems: "center", gap: 7,
                padding: "6px 18px", borderRadius: 999,
                background: `${G}12`, border: `1px solid ${G}28`,
                color: G, fontSize: 12, fontWeight: 900,
                letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 28,
              }}>
                <Sparkles size={12} /> Tanzania's Digital Ecosystem
              </div>
            </motion.div>

            {/* Headline */}
            <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08, duration: 0.62, ease: SPRING }}>
              <h1 style={{
                fontFamily: "'Bricolage Grotesque',sans-serif",
                fontSize: isMobile ? "clamp(36px,11vw,52px)" : "clamp(52px,7vw,80px)",
                fontWeight: 900, lineHeight: 1.06, letterSpacing: "-.055em",
                margin: "0 0 12px",
                textShadow: "0 4px 48px rgba(0,0,0,.5)",
              }}>
                One Platform.<br />
            ...
            {/* Subtext */}
            <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.17, duration: 0.58, ease: SPRING }}
              style={{ color: "rgba(255,255,255,.58)", fontSize: isMobile ? 15 : 18, maxWidth: 580, margin: "0 auto 36px", lineHeight: 1.78 }}>
              Student tools, tech growth, business services, and shopping — built for Tanzania, open to the world.
            </motion.p>

            {/* CTAs */}
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.55, ease: SPRING }}
              style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginBottom: 56 }}>
              <GoldBtn onClick={scrollToSectors} icon={<Sparkles size={15} />}>
                Get Started
              </GoldBtn>
              ...
              {deferredPrompt && !isInstalled && isMobile && (
                <GoldBtn onClick={installApp} style={{ background: "#22c55e", boxShadow: "0 6px 22px rgba(34,197,94,.35)" }}>
            ...
            {/* Stat chips */}
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.55, ease: SPRING }}
              style={{ display: "flex", gap: isMobile ? 10 : 16, flexWrap: "wrap", justifyContent: "center" }}>
              {[["10K+","Students"],["4","Core Sectors"],["Free","To Access"],["🇹🇿","Tanzania"]].map(([n, l]) => (
                <div key={l} style={{
                  ...
                  <div style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontWeight: 900, fontSize: isMobile ? 16 : 18, color: G, lineHeight: 1 }}>{n}</div>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,.38)", fontWeight: 700, marginTop: 3 }}>{l}</div>
                </div>
              ))}
            </motion.div>
...
function SectorCard({ sector, goPage, index }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay: index * 0.06, duration: 0.5, ease: SPRING }}
      onClick={() => goPage(sector.path)}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{
        flexShrink: 0,
        width: isMobile ? "clamp(240px,72vw,280px)" : "100%",
        scrollSnapAlign: "start",
        background: hov
          ? `linear-gradient(145deg,rgba(255,255,255,.08),rgba(255,255,255,.03))`
          : `linear-gradient(145deg,rgba(255,255,255,.055),rgba(255,255,255,.02))`,
        backdropFilter: "blur(16px)",
        border: `1px solid ${hov ? sector.color + "45" : "rgba(255,255,255,.08)"}`,
        borderRadius: 24,
        ...
        boxShadow: hov
          ? `0 20px 56px rgba(0,0,0,.5), 0 0 0 1px ${sector.color}22, inset 0 1px 0 rgba(255,255,255,.1)`
          : "0 4px 24px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.06)",
    ...
    >
      ...
      <div style={{ position: "absolute", top: -30, right: -30, width: 140, height: 140, borderRadius: "50%",
        ...
        filter: "blur(28px)", pointerEvents: "none", opacity: hov ? 1 : 0.5, transition: "opacity 0.3s" }} />

      {/* Tag */}
      <div style={{ position: "absolute", top: 16, right: 16, padding: "3px 10px", borderRadius: 999,
        background: `${sector.color}16`, color: sector.color,
      ...
      <div style={{
        ...
        background: `${sector.color}14`, border: `1px solid ${sector.color}22`,
        display: "flex", alignItems: "center", justifyContent: "center",
        color: sector.color, marginBottom: 18,
        transform: hov ? "scale(1.1)" : "scale(1)", transition: "transform 0.24s ease",
      }}>
        {sector.icon}
      </div>

      <h3 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: isMobile ? 18 : 20,
        fontWeight: 900, letterSpacing: "-.025em", margin: "0 0 8px", lineHeight: 1.2 }}>
        {sector.title}
      </h3>
      <p style={{ color: "rgba(255,255,255,.48)", fontSize: 13.5, lineHeight: 1.65, margin: "0 0 18px" }}>
        {sector.desc}
      </p>

      <div style={{ display: "flex", alignItems: "center", gap: 5, color: sector.color, fontWeight: 800, fontSize: 13 }}>
        Open <ChevronRight size={14} style={{ transform: hov ? "translateX(4px)" : "", transition: "transform 0.2s" }} />
      </div>
    </motion.div>
  );
}
...
export default function HomePage({ goPage }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: "#fff", fontFamily: "'Instrument Sans',system-ui,sans-serif", overflowX: "hidden" }}>
      ...
      {/* ── 5. MAIN SECTORS ───────────────────────── */}
      <section ref={sectorsRef} style={{ padding: "clamp(48px,7vw,80px) 0" }}>
        <W>
          <SectionHead
            label="Platform" labelIcon={<Globe size={11} />}
            title="Choose Your Sector"
            desc="Four focused hubs, each built for a specific purpose."
          />
        </W>
        ...
        {isMobile ? (
          <HScrollRow gap={14}>
            {SECTORS.map((s, i) => <SectorCard key={s.id} sector={s} goPage={goPage} index={i} />)}
          </HScrollRow>
        ) : (
          <W>
      ...
      {/* ── 6b. POPULAR IN STUDENT CENTER ─────────── */}
      {(coursesLoading || courses.length > 0) && (
        <section style={{ padding: "clamp(32px,5vw,56px) 0", borderTop: "1px solid rgba(255,255,255,.05)", background: "rgba(16,185,129,.025)" }}>
          <W>
            <SectionHead
              label="Student Center" labelColor="#10b981" labelIcon={<GraduationCap size={11} />}
              title="Popular Courses & Resources"
              action={() => goPage("exams")} actionText="Explore"
            />
          </W>
          <HScrollRow gap={14}>
            {coursesLoading
              ...
              : courses.map(item => (
                <div key={item.id} onClick={() => goPage("exams")} style={{
                  flexShrink: 0, width: "clamp(200px,58vw,240px)", scrollSnapAlign: "start",
                  cursor: "pointer", borderRadius: 18, overflow: "hidden",
                  background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)",
                }}>
                  <div style={{ height: 130, background: "rgba(255,255,255,.06)", position: "relative" }}>
                    {(item.imageUrl || item.image) && <img src={item.imageUrl || item.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} referrerPolicy="no-referrer" />}
                    <div style={{ position: "absolute", bottom: 8, left: 10 }}>
                      <span style={{ fontSize: 9, fontWeight: 900, padding: "3px 8px", borderRadius: 6, background: "rgba(16,185,129,.8)", color: "#fff", textTransform: "uppercase" }}>
                        {item.category || "Course"}
                      </span>
                    </div>
                  </div>
                  <div style={{ padding: "12px 14px" }}>
                    <h4 style={{ fontSize: 13.5, fontWeight: 800, margin: "0 0 4px", lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{item.title || item.name}</h4>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,.35)", fontWeight: 700 }}>{item.lessons || "0"} Lessons · {item.duration || "Self-paced"}</div>
                  </div>
                </div>
              ))
            }
      ...
      {(productsLoading || products.length > 0) && (
        <section style={{ padding: "clamp(32px,5vw,56px) 0", borderTop: "1px solid rgba(255,255,255,.05)", background: `rgba(245,166,35,.02)` }}>
          ...
          <HScrollRow gap={14}>
            {productsLoading
              ? [1,2,3,4].map(i => <div key={i} style={{ flexShrink:0, width:180, scrollSnapAlign:"start" }}><PostSkeleton /></div>)
              : products.map(raw => {
                const item = normalizeItem(raw);
                return (
                  <div key={raw.id} onClick={() => goPage("duka/phones")} style={{
                    flexShrink: 0, width: "clamp(150px,45vw,190px)", scrollSnapAlign: "start",
                    cursor: "pointer", borderRadius: 16, overflow: "hidden",
                    background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.07)",
                  }}>
                    <div style={{ height: 130, background: "rgba(255,255,255,.06)" }}>
                      {item.image && <img src={item.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} referrerPolicy="no-referrer" />}
                    </div>
                    <div style={{ padding: "10px 12px" }}>
                      <div style={{ fontSize: 12.5, fontWeight: 800, marginBottom: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.title}</div>
                      <div style={{ fontSize: 12, color: G, fontWeight: 900 }}>{raw.price ? `${raw.price} TZS` : "Contact"}</div>
                    </div>
                  </div>
                );
              })
      ...
      {updates.length > 0 && (
        <section style={{ padding: "clamp(32px,5vw,56px) 0", borderTop: "1px solid rgba(255,255,255,.05)" }}>
          ...
          <HScrollRow gap={14}>
            {updates.map(item => <ContentCard key={item.id} item={item} onClick={() => goPage("tips")} />)}
          </HScrollRow>
        </section>
      )}
