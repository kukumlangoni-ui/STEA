      ...
      <section style={{ 
        ...
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 16px", background: "rgba(245,166,35,0.1)", borderRadius: 20, color: G, fontSize: 14, fontWeight: 700, marginBottom: 24 }}>
            <BookOpen size={16} /> {t('exams_hero_tag')}
          </div>
          ...
          <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap:"wrap" }}>
            <Link to="/exams/results" style={{ 
              ...
              {t('exams_action_results')} <ArrowRight size={18} />
            </Link>
      ...
      <section style={{ maxWidth: 1200, margin: "80px auto 0", padding: "0 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 20 }}>
          <CategoryCard 
            ...
            title={t('exams_cat_necta_title')}
            desc={t('exams_cat_necta_desc')}
            color="#4ade80"
          />
          <CategoryCard 
            to="/exams/past-papers"
            icon={<FileText size={32} />}
            title={t('exams_cat_past_title')}
            desc={t('exams_cat_past_desc')}
            color="#60a5fa"
          />
          <CategoryCard 
            to="/exams/notes"
            icon={<BookOpen size={32} />}
            title={t('exams_cat_notes_title')}
            desc={t('exams_cat_notes_desc')}
            color="#facc15"
          />
          <CategoryCard 
            to="/exams/practice"
            icon={<Star size={32} />}
            title={t('exams_cat_practice_title')}
            desc={t('exams_cat_practice_desc')}
            color="#f472b6"
          />
          <CategoryCard 
            to="/courses"
            icon={<BookOpen size={32} />}
            title={t('exams_cat_courses_title')}
            desc={t('exams_cat_courses_desc')}
            color="#3b82f6"
          />
          <CategoryCard 
            to="/university-guide"
            icon={<BookOpen size={32} />}
            title={t('exams_cat_uni_title')}
            desc={t('exams_cat_uni_desc')}
            color="#a855f7"
          />
        </div>
      </section>
      ...
      {featured.length > 0 && (
        <section style={{ maxWidth: 1200, margin: "80px auto 0", padding: "0 20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
            <div>
              <h2 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 28, fontWeight: 800, marginBottom: 8 }}>{t('exams_featured_title')}</h2>
              <p style={{ color: "rgba(255,255,255,0.5)" }}>{t('exams_featured_desc')}</p>
            </div>
            <Link to="/exams/past-papers" style={{ color: G, textDecoration: "none", fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}>
              {t('exams_action_see_all')} <ChevronRight size={16} />
            </Link>
          </div>
