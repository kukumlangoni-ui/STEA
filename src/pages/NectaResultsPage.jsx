...
export default function NectaResultsPage() {
  ...
  const handleSearch = async (schoolCode, schoolName) => {
    ...
    if (parsedYear > currentYear) {
      setYearError("Results for this year are not yet available. Please try again later.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Results not found. Please check the year and exam type.");
      }
      const data = await res.json();
      if (!data.students || data.students.length === 0) {
        throw new Error("No results found for this school in the selected year.");
      }
      setResults(data);
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: DARK, color: "#fff", paddingBottom: 100, overflowX: "hidden" }}>
      {/* Hero Section */}
      <section style={{ 
        ...
        >
          <div style={{ display: "flex", justifyContent: "center", gap: 12, marginBottom: 24 }}>
            <button onClick={() => window.location.href = "/exams"} style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", padding: "8px 16px", borderRadius: 12, color: "rgba(255,255,255,0.6)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
              <ArrowLeft size={16} /> Student Center
            </button>
