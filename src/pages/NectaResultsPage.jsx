...
export default function NectaResultsPage() {
  ...
  useEffect(() => {
    ...
    // If it looks like a school code (e.g. S0155), we might want to search specifically for that
    // but the backend already handles both name and code.
  ...
  const getCaptureCanvas = async (elementRef) => {
    ...
    return await html2canvas(elementRef.current, {
      ...
      onclone: (doc) => {
        const studentEl = doc.getElementById('student-result-card');
        if (studentEl) {
          studentEl.style.width = '600px';
          studentEl.style.maxWidth = 'none';
          studentEl.style.height = 'auto';
          studentEl.style.transform = 'none';
          studentEl.style.position = 'relative';
          studentEl.style.margin = '0';
          studentEl.style.overflow = 'visible';
        }
        const schoolEl = doc.getElementById('school-result-card');
        if (schoolEl) {
          schoolEl.style.width = '1000px';
          schoolEl.style.maxWidth = 'none';
        }
      }
    });
  };
  ...
  const downloadPDF = async (elementRef, filename) => {
    const canvas = await getCaptureCanvas(elementRef);
    if (!canvas) return;
            ...
            <div style={{ 
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 20 }}>
                ...
                <div data-html2canvas-ignore="true" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  ...
                  <button 
                    onClick={() => downloadPDF(resultsRef, `NECTA_${results.schoolCode}_${year}`)}
          ...
          <div style={{ 
            ...
            >
              ...
              <div data-html2canvas-ignore="true" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <button 
                  onClick={() => downloadPDF(studentRef, `Result_${selectedStudent.indexNumber}`)}
                  style={actionButtonStyle}
                >
                  <Download size={18} /> PDF
                </button>
                <button 
                  onClick={() => downloadImage(studentRef, `Result_${selectedStudent.indexNumber}`)}
                  style={actionButtonStyle}
                >
