...
export default function NectaResultsPage() {
  ...
  const doSearch = async (schoolCode, schoolName) => {
    ...
    if (isNaN(py) || py < 1990) { setYearError("Tafadhali weka mwaka sahihi (mfano: 2024)."); return; }
    if (py > CURRENT_YEAR) { setYearError("Matokeo ya mwaka huu bado hayajatoka. Tafadhali jaribu tena baadaye."); return; }
    ...
    try {
      ...
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error || "Matokeo hayajapatikana. Hakikisha mwaka na aina ya mtihani ni sahihi."); }
      const data = await res.json();
      if (!data.students || data.students.length === 0) throw new Error("Hakuna matokeo yaliyopatikana kwa shule hii katika mwaka huu.");
      setResults(data);
