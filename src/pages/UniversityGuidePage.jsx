...
function PointsCalculator() {
  ...
  const GRADE_POINTS = { A:5, B:4, C:3, D:2, E:1, S:0.5, F:0 };
  const [subjects, setSubjects] = useState([
    { name:"Somo la 1 (Principal)", grade:"—" },
    { name:"Somo la 2 (Principal)", grade:"—" },
    { name:"Somo la 3 (Principal)", grade:"—" },
    { name:"Somo la 4 (Principal)", grade:"—" },
    { name:"Subsidiary", grade:"—" },
  ]);

  const principalGrades = subjects.slice(0,4).map(s => GRADE_POINTS[s.grade] ?? null).filter(p => p !== null);
  const top3 = [...principalGrades].sort((a,b) => b-a).slice(0,3);
  const total = top3.reduce((sum,p) => sum+p, 0);
  const hasEnough = top3.length === 3;

  const getCategory = (pts) => {
    if (pts >= 15) return { label:"Excellent — Daktari, Engineering ya juu", color:"#4ade80" };
    if (pts >= 12) return { label:"Very Good — Engineering, Computer Science", color:"#60a5fa" };
    if (pts >= 9)  return { label:"Good — Law, Science, Business", color:G };
    if (pts >= 6)  return { label:"Average — Education, Social Sciences", color:"#fb923c" };
    return { label:"Jaribu tena au chagua programu za chini", color:"#f87171" };
  };

  return (
    <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:20, padding:"22px 22px" }}>
      <h3 style={{ fontSize:15, fontWeight:800, marginBottom:6 }}>🧮 Hesabu Pointi Zako Mwenyewe</h3>
      <p style={{ fontSize:13, color:"rgba(255,255,255,.45)", marginBottom:20, lineHeight:1.6 }}>
        Chagua daraja kwa kila somo lako. Mfumo utahesabu jumla ya pointi 3 bora.
      </p>
      <div style={{ display:"grid", gap:10, marginBottom:20 }}>
        {subjects.map((s, i) => (
          <div key={i} style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
            <div style={{ flex:1, fontSize:13.5, fontWeight:700, color:"rgba(255,255,255,.7)", minWidth:140 }}>{s.name}</div>
            <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
              {GRADES.map(g => (
                <button key={g} onClick={() => setSubjects(prev => prev.map((sub,j) => j===i ? {...sub, grade:g} : sub))}
                  style={{ width:34, height:34, borderRadius:10, border:`1.5px solid ${s.grade===g ? (g==="A"?"#4ade80":g==="B"?"#60a5fa":g==="C"?G:g==="D"?"#fb923c":g==="F"?"#f87171":BORDER) : BORDER}`, background:s.grade===g?"rgba(255,255,255,.08)":"transparent", color:s.grade===g?"#fff":"rgba(255,255,255,.35)", fontWeight:800, fontSize:12, cursor:"pointer", transition:"all .15s" }}>
                  {g}
                </button>
              ))}
            </div>
      ...
      {hasEnough ? (
        <div style={{ padding:"18px 20px", borderRadius:16, background:`${getCategory(total).color}12`, border:`1px solid ${getCategory(total).color}30` }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
            <div style={{ fontFamily:"'Bricolage Grotesque',sans-serif", fontSize:38, fontWeight:900, color:getCategory(total).color }}>{total}</div>
            <div>
              <div style={{ fontSize:12, fontWeight:900, color:"rgba(255,255,255,.4)", textTransform:"uppercase", letterSpacing:".07em" }}>Jumla ya Pointi (3 Bora)</div>
              <div style={{ fontSize:13.5, fontWeight:700, color:getCategory(total).color, marginTop:3 }}>{getCategory(total).label}</div>
            </div>
          </div>
          <div style={{ fontSize:12, color:"rgba(255,255,255,.45)" }}>
            Pointi {top3.map(p => `${p}`).join(" + ")} = <strong style={{ color:"#fff" }}>{total}</strong>
          </div>
        </div>
      ) : (
        <div style={{ padding:"14px 18px", borderRadius:14, background:"rgba(255,255,255,.03)", border:`1px solid ${BORDER}`, textAlign:"center" }}>
          <p style={{ fontSize:13, color:"rgba(255,255,255,.4)", margin:0 }}>Chagua daraja kwa masomo yako 4 ya Principal ili uone jumla yako ya pointi.</p>
        </div>
      )}
    </div>
  );
...
export default function UniversityGuidePage() {
  ...
  const TABS = [
    ...
    { id:"points",       label:"Pointi Form 6",    icon:<Trophy size={15}/> },
          ...
          {tab==="overview" && (
            <motion.div key="overview" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))", gap:14, marginBottom:52 }}>
                {[
                  { icon:<Trophy size={22}/>,      title:"Pointi Form 6",       desc:"Hesabu pointi za ACSEE",             tab:"points",       color:G },
