...
function PointsCalculator() {
  ...
  const handleCalculate = () => {
    ...
    const interp = (pts) => {
      if (pts >= 15) return { emoji:"🏆", label:"Bora Sana!",    path:"Unaweza kuomba Medicine, Engineering ya hali ya juu, Computer Science — UDSM, MUHAS, ARU, NM-AIST.", color:"#4ade80" };
      if (pts >= 12) return { emoji:"🎯", label:"Vizuri Sana",   path:"Unaweza kuomba Engineering, Computer Science, Law, Science — UDSM, ARU, UDOM, SAUT.",               color:"#60a5fa" };
      if (pts >= 9)  return { emoji:"👍", label:"Vizuri",        path:"Unaweza kuomba Business, Accountancy, Education ya sayansi, Social Sciences — Mzumbe, IFM, SAUT.",   color:G         };
      if (pts >= 6)  return { emoji:"📚", label:"Wastani",       path:"Unaweza kuomba Education, Social Work, programu za OUT — zinazokubali pointi za chini.",             color:"#fb923c"  };
      return              { emoji:"💪", label:"Jaribu Tena",   path:"Pointi chache — fikiria retake au programu za certificate/diploma kwanza.",                           color:"#f87171"  };
    };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:20, padding:"18px 20px" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
          {[...Object.keys(COMBO_SUBJECTS),"Zote"].map(c => {
            const active = c === "Zote" ? combo === "" : combo === c;
            return (
              <button key={c} onClick={() => { setCombo(c === "Zote" ? "" : c); setCalculated(false); setResult(null); }}
      ...
      <button onClick={handleCalculate} disabled={!canCalculate}
        ...
        {canCalculate ? "🧮  Hesabu Pointi Zangu" : "Weka masomo 3 ya Principal na daraja zao kwanza"}
      </button>
