...
import {
  ...
  ChevronDown, ArrowRight, Menu, Banknote,
} from "lucide-react";
  ...
  },
  {
    ...
    cols: [
      ...
      },
      {
        heading: "Dijiti",
        items: [
          { label: "Abroad Money Guide", desc: "Mwongozo wa kutuma pesa nje", path: "/money-guide", icon: <Banknote size={15} />, hot: true },
        ...
        <>
          ...
          <motion.div
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 12px" }}>
              ...
              {/* Simple path items — no dropdown */}
              {[
                { to:"/vpn",   emoji:"🔒", label:"VPN Guide" },
                { to:"/gigs",  emoji:"💼", label:"Gigs & Kazi" },
              ].map(item => (
                <Link key={item.to} to={item.to} onClick={onClose}
                  style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", borderRadius:12, color:"#fff", textDecoration:"none", fontWeight:700, fontSize:15, marginBottom:2, transition:"background .15s" }}
                  onMouseEnter={e => e.currentTarget.style.background="rgba(255,255,255,.05)"}
                  onMouseLeave={e => e.currentTarget.style.background="transparent"}>
