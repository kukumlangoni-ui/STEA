...
const getNav = (t) => [
  { id: "home", label: t('nav_home'), path: "/" },
  {
    ...
    cols: [
      ...
      },
      {
        ...
        items: [
          { label: t('nav_tech_web_label'), desc: t('nav_tech_web_desc'), path: "/websites", icon: <Globe size={15} /> },
  ...
  },
  {
    ...
    featured: { title: t('nav_duka_title'), desc: t('nav_duka_desc'), path: "/duka", icon: "🛍️" },
    cols: [
      {
        ...
        {open && (
          <motion.div initial={{ opacity: 0, y: 6, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 6, scale: .96 }}
            ...
            <div style={{ padding: "12px 14px", borderBottom: "1px solid rgba(255,255,255,.06)", marginBottom: 6 }}>
              ...
              {(user.role === "admin" || user.role === "manager" || user.role === "seller" || user.role === "creator" || user.role === "reviewer") && (
