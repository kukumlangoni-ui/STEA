...
async function startServer() {
  ...
  app.get("/api/necta/schools", async (req, res) => {
    ...
    try {
      // NECTA URL patterns vary by year and exam type
      // Recent years (2020+): https://onlinesys.necta.go.tz/results/2024/csee/index.htm
      // Older years might differ, but we'll focus on 2020+ for now
      const baseUrl = `https://onlinesys.necta.go.tz/results/${year}/${String(examType).toLowerCase()}/index.htm`;
      ...
      res.status(500).json({ error: "Failed to fetch schools from NECTA. The results for this year might not be available yet." });
    }
  });
  ...
  app.get("/api/necta/results/:examType/:year/:schoolCode", async (req, res) => {
    ...
    // URL pattern: https://onlinesys.necta.go.tz/results/2024/csee/results/s0155.htm
    ...
    try {
      ...
      tables.each((i, table) => {
        ...
        // Usually has "I", "II", "III", "IV", "0" or "DISTINCTION", "MERIT"
      ...
      res.status(500).json({ 
        error: "Failed to fetch results from NECTA.",
